<template>
  <div
    ...
    :class="{ 'opacity-100': sidebarResizing }" @mousedown="startResize" />
  <div v-show="sidebarResizing"
    class="fixed z-20 mt-3 -translate-x-[130%] rounded-md bg-gray-800 px-2 py-1 text-base text-white"
    :style="{ left: sidebarWidth + 'px' }">
    {{ sidebarWidth }}
  </div>

  <div class="inline-flex h-full flex-1 flex-col overflow-auto border-r bg-gray-50 pb-40"
    ...
    <div class="flex-1">
      <nav class="space-y-0.5 px-2">
        <Links :links="navigation" class="flex items-center rounded px-2 py-1 text-gray-800 transition"
          active="bg-white shadow-sm" inactive="hover:bg-gray-100">
        ...
        <button v-if="$user().isNotGuest" class="flex w-full items-center rounded px-2 py-1 text-gray-800"
      ...
      <nav class="mt-1 space-y-0.5 px-2">
        <div v-for="team in activeTeams" :key="team.name">
          ...
          <button @click.prevent="() => {
            ...
            <ChevronTriangle class="h-3 w-3 text-gray-500 transition duration-200"
          ...
          <div class="mb-2 mt-0.5 space-y-0.5 pl-7" v-show="team.open">
            <Link :key="project.name" v-for="project in teamProjects(team.name)" :link="project"
              :ref="($comp) => setProjectRef($comp, project)"
              class="flex h-7 items-center rounded-md px-2 text-gray-800 transition" active="bg-white shadow-sm"
            ...
            <div class="flex h-7 items-center px-2 text-sm text-gray-600" v-if="teamProjects(team.name).length === 0">
              No projects
            </div>
          </div>
        </div>
